...
export const updateDocumentDownload = async (nombreDocumento, idAlumno, ipUsuario) => {
  try {
    const getIdDocumento = await executeQuery(`SELECT id_documento FROM documentos_alumno WHERE id_alumno = ${idAlumno} AND enlace = "${nombreDocumento}"`)
