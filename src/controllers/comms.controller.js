...
export const getAllCommsReceived = async (req, res) => {
    try {
        if(req.query.user_id && req.query.tipo_destino) {
            const query = `SELECT * FROM comunicaciones, comunicaciones_destinos WHERE comunicaciones_destinos.iddestino = ${req.query.user_id} AND comunicaciones_destinos.tipodestino = ${req.query.tipo_destino} HAVING comunicaciones_destinos.idcomunicacion = comunicaciones.idcomunicacion`
...
export const getAllCommsSent = async (req, res) => {
    try {
        if(req.query.user_id && req.query.tipo_remite) {
            const query = `SELECT * FROM comunicaciones, comunicaciones_destinos WHERE comunicaciones.idremite = ${req.query.user_id}  AND comunicaciones.tiporemite = ${req.query.tipo_remite} HAVING comunicaciones_destinos.idcomunicacion = comunicaciones.idcomunicacion`
            const result = await executeQuery(query)
            if(result.length) {
                let comms = []
                for (let index = 0; index < result.length; index++) {
