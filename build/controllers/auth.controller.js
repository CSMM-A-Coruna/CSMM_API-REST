...
var tipoUsuarios = ['administradores', 'profesores', 'alumnos', 'familias']; // Registro
      ...
      while (1) {
        switch (_context2.prev = _context2.next) {
          case 0:
            ...
            tipoUsuarios.forEach(function (tipo) {
              _database.db.query("SELECT * FROM ".concat(tipo, " WHERE usuario = ?"), req.body.usuario, function (err, result) {
                // Comprobamos que hay resultado
                if (result.length) {
                  // Verificamos la contraseña
                  var verifyPassword = _Usuario["default"].compareSyncPassword(req.body.password, result[0].password);

                  if (verifyPassword) {
                    var alumnosResult = _Usuario["default"].calcularAlumnosAsociados(result[0].id).then(function (data) {
                      if (data.length) {
                        var alumnos = [];
